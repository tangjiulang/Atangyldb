...
int main(int argc, char *argv[])
{
  ...
  char input[5][MAX_MEM_BUFFER_SIZE] = {
    {"Create Table na1 (id int, age int);"},
    {"Create index in1 on na1 (id);"},
    {"Insert into na1 values(10, 20);"},
    {"Insert into na1 values(5, 10);"},
    {"Select * from na1;"}
  };
  int max = 5;
  char *input_command = nullptr;
  for (int i = 0; i < max; i++) {
    if ((send_bytes = write(sockfd, input[i], strlen(input[i]) + 1)) == -1) {
